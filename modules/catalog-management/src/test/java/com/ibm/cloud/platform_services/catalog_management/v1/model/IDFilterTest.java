...
/**
 * Unit test class for the IDFilter model.
 */
public class IDFilterTest {
  ...
  @Test
  public void testIDFilter() throws Throwable {
    ...
    String json = TestUtilities.serialize(idFilterModel);

    IDFilter idFilterModelNew = TestUtilities.deserialize(json, IDFilter.class);
    assertTrue(idFilterModelNew instanceof IDFilter);
    assertEquals(idFilterModelNew.include().toString(), filterTermsModel.toString());
    assertEquals(idFilterModelNew.exclude().toString(), filterTermsModel.toString());
  }
}