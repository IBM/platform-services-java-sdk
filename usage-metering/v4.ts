...
class UsageMeteringV4 extends BaseService {
   ...
   * @param {ResourceInstanceUsage[]} params.resourceInstanceUsage -
  ...
  public reportResourceUsage(params: UsageMeteringV4.ReportResourceUsageParams): Promise<UsageMeteringV4.Response<UsageMeteringV4.ResponseAccepted>> {
    ...
    const requiredParams = ['resourceId', 'resourceInstanceUsage'];
    ...
    return new Promise((resolve, reject) => {
      ...
      const body = _params.resourceInstanceUsage;
      const path = {
   ...
   * @param {Object} params - The parameters to send to the service.
   * @param {string} params.resourceId - The resource for which the usage is submitted.
   * @param {CfResourceInstanceUsage[]} params.cfResourceInstanceUsage -
   * @param {OutgoingHttpHeaders} [params.headers] - Custom request headers
   * @returns {Promise<UsageMeteringV4.Response<UsageMeteringV4.ResponseAccepted>>}
   */
  public reportCfresourceUsage(params: UsageMeteringV4.ReportCfresourceUsageParams): Promise<UsageMeteringV4.Response<UsageMeteringV4.ResponseAccepted>> {
    const _params = extend({}, params);
    const requiredParams = ['resourceId', 'cfResourceInstanceUsage'];
    ...
    return new Promise((resolve, reject) => {
      ...
      const body = _params.cfResourceInstanceUsage;
      const path = {
        'resource_id': _params.resourceId
      };

      const sdkHeaders = getSdkHeaders(UsageMeteringV4.DEFAULT_SERVICE_NAME, 'v4', 'reportCfresourceUsage');

      const parameters = {
        options: {
          url: '/v1/metering/resources/{resource_id}/usage',
          method: 'POST',
          body,
          path,
        },
...
namespace UsageMeteringV4 {
  ...
  /** Parameters for the `reportCfresourceUsage` operation. */
  export interface ReportCfresourceUsageParams {
    /** The resource for which the usage is submitted. */
    resourceId: string;
  ...
  export interface CfResourceInstanceUsage {
    /** The ID of the orgnization to which the instance belongs. If the usage is for the instance is in a public
     *  cloud region, then the organization ID format is `{mccp_region_name}:{guid}`. If the usage is for a syndicated
     *  instance, then the organization ID format is `{mccp_region_id}:{guid}`.
     */
    ...
    plan_id: string;
    /** The pricing region for the usage. For public regions, the region parameter value is the mccp region name in
     *  which the organization is present. For syndicated usage, use `us-south`.
     */
    region: string;
